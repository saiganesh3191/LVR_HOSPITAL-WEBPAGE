...
export function DoctorCard({ doctor }: { doctor: Doctor }) {
  return <article className={`doctor-card doctor-card-with-photo ${doctor.color}`}><Link className="portrait-link" href={`/doctors/${doctor.slug}`} aria-label={`View ${doctor.name}'s profile`}><DoctorPortrait doctor={doctor} /></Link><div className="doctor-card-copy"><h3><Link href={`/doctors/${doctor.slug}`}>{doctor.name}</Link></h3><span className="doctor-qualification">{doctor.qualification}</span><div className="doctor-summary"><span><small>OP consultation</small><strong>₹{profileDetails[doctor.slug]?.fee}</strong></span><span><small>Consultation</small><strong>{doctor.appointmentType === "Consultant" ? "Visiting specialist" : "Regular clinic"}</strong></span></div><p><Clock3 size={15} /><span>{doctor.timing}<small>Call reception to confirm availability</small></span></p><div className="doctor-card-actions"><Link className="doctor-profile-link" href={`/doctors/${doctor.slug}`}>View profile <ArrowUpRight size={16} /></Link><Link className="text-link" href={`/appointment?doctor=${doctor.slug}`}>Request appointment <ArrowRight size={16} /></Link></div></div></article>;
}
